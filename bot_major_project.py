...
updater = Updater(token='1157584172:AAE8RSHk7yklJHM4IpvzcjGzH8jlcdEF0zA', use_context=True)
dispatcher = updater.dispatcher

def start(update, context):
    context.bot.send_message(chat_id=update.effective_chat.id, text="I'm a bot, say hi to me")

def stop(update, context):
    context.bot.send_message(chat_id=update.effective_chat.id, text="Have a wonderful day!")

def weatherInfo(update, context):
    context.bot.send_message(chat_id=update.effective_chat.id, text="Wait! We are waiting for the results")

    response = req.get("https://api.openweathermap.org/data/2.5/weather?q=rajasthan&appid=37643fffe08d4b8b453d5781ef356c29")
    ...
    context.bot.send_message(chat_id=update.effective_chat.id, text="Temperature of rajasthan is: "+temperature+"\nHumidity in rajasthan is: "+ humidity)
    # context.bot.send_message(chat_id=update.effective_chat.id, text="Humidity of rajasthan is %s"%humidity)
